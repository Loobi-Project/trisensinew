...
export default function Classes({ classes = [] }: ClassesProps) {
    const { data, setData, post, reset, errors } = useForm({ name: '' });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('staffadmin.store-class'), {
            preserveScroll: true,
            onSuccess: () => {
                reset();
    ...
    const handleEdit = (classes: Classes) => {
        setEditId(classes.id);
        setEditName(classes.name);
    };
    ...
    const handleUpdate = (e: React.FormEvent) => {
        ...
        router.put(
            route('staffadmin.update-class', editId),
            { name: editName },
            {
                preserveScroll: true,
                onSuccess: () => {
                    ...
                    setSortedClasses((prevClasses) => prevClasses.map((cls) => (cls.id === editId ? { ...cls, name: editName } : cls)));
                },
                onError: (errors) => {
                    console.error('Update errors:', errors);
                    setIsLoading(false);
                    showNotification('Gagal memperbarui kelas', 'error');
                },
            },
        );
    };
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('staffadmin.destroy-class', editId), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    useEffect(() => {
        if (classes) {
            const sorted = [...classes].sort((a, b) => b.name - a.name);
    ...
    const paginatedClassrooms = sortedClasses.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
        ...
        <AppLayoutStaffAdmin breadcrumbs={breadcrumbs}>
                        ...
                        <div className="relative mt-6 min-h-[80vh] flex-1 overflow-hidden rounded-xl border border-gray-200 bg-white p-4 text-gray-900 shadow-sm md:min-h-min md:p-6 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                            ...
                            <div className="space-y-8">
                                <div className="flex flex-col border-b border-gray-200 pb-5 md:flex-row md:items-center md:justify-between dark:border-gray-700">
                                    <h2 className="mb-4 text-xl font-bold md:mb-0 md:text-2xl">Manajemen Kelas</h2>
                                    ...
                                    <table className="table w-full">
                                        ...
                                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                            <AnimatePresence initial={false}>
                                                {paginatedClassrooms.filter((classroom) => classroom.id && classroom.name).length > 0 ? (
                                                    ...
                                                    <tr className="bg-white dark:bg-gray-800">
                                                        <td colSpan={3} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                                            <div className="flex flex-col items-center justify-center space-y-3">
                                                                ...
                                                                <span className="font-medium">Belum ada data kelas</span>
                                                                <p className="text-sm">Tambahkan kelas untuk mulai menggunakan fitur ini</p>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                )}
                                ...
                                {sortedClasses.length > itemsPerPage && (
                                    <div className="mt-4 flex items-center justify-between">
                                        <div className="text-sm text-gray-600 dark:text-gray-400">
                                            Menampilkan {Math.min(sortedClasses.length, (currentPage - 1) * itemsPerPage + 1)} -{' '}
                                            {Math.min(currentPage * itemsPerPage, sortedClasses.length)} dari {sortedClasses.length} data
                                        </div>
            ...
            <dialog id="delete-modal" className="modal">
                <div className="modal-box bg-white text-gray-900 shadow-lg dark:bg-gray-800 dark:text-white">
                    ...
                    <p className="py-4">Apakah Anda yakin ingin menghapus kelas ini? Tindakan ini tidak dapat dibatalkan.</p>
