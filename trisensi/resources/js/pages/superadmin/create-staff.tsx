...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Buat Akun Tata Usaha', href: '/superadmin/spadm' },
];
...
export default function CreateStaff() {
    ...
    const fetchStaffUsers = async (signal?: AbortSignal) => {
        try {
            const response = await axios.get('/superadmin/spadm/get-staff', { signal });
    ...
    const submitForm = (e: React.FormEvent) => {
        e.preventDefault();
        post('/superadmin/spadm/create-staff', {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const confirmDelete = async () => {
        try {
            ...
            setTimeout(async () => {
                await axios.post('/superadmin/spadm/delete-staff-multiple', { ids: Array.from(selectedUsers) });
    ...
    return (
        <AppLayoutSuperAdmin breadcrumbs={breadcrumbs}>
            ...
            <div className="bg-gray-50 p-3 sm:p-4 md:p-6 dark:bg-gray-900">
                <div className="rounded-xl border bg-white p-4 shadow-md sm:p-6 dark:bg-gray-800">
                    <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl md:text-3xl dark:text-white">Buat Akun Tata Usaha</h1>
                    <p className="mt-2 text-sm text-gray-600 sm:text-base dark:text-gray-400">Gunakan formulir ini untuk menambahkan akun baru.</p>

                    {/* Form responsive */}
                    <form onSubmit={submitForm} className="mt-4 space-y-3 md:mt-6 md:space-y-4">
                        ...
                        <div>
                            ...
                            />
                            {errors.password && <p className="text-xs text-red-500 sm:text-sm">{errors.password}</p>}
                        </div>

                        <div className="mt-4 flex w-full flex-col items-center justify-between gap-2 sm:flex-row md:mt-6">
                            <motion.button
                                onClick={handleDeleteSelected}
                                onKeyDown={(e) => e.key === 'Enter' && handleDeleteSelected()}
                                className="w-full rounded-lg bg-red-600 px-3 py-2 text-sm text-white hover:bg-red-700 sm:w-auto md:text-base"
                            ...
                            <button
                                type="submit"
                                className="w-full rounded-lg bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700 sm:w-auto md:text-base"
                                disabled={processing}
                            >
                                ...
                                <div className="overflow-hidden rounded-lg border">
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                            <tr>
                                                <th scope="col" className="p-2 text-center sm:p-3">
                                                    <motion.div whileTap={{ scale: 0.95 }}>
                                                        <input
                                                            type="checkbox"
                                                            onChange={(e) => {
                                                                // Filter hanya staff yang memiliki relasi dengan AdminStaff
                                                                const staffWithAdminConnection = users.filter((staff) => staff.admin_staff); // Asumsikan bahwa data staff sudah termasuk relasi admin_staff

                                                                setSelectedUsers(
                                                                    e.target.checked
                                                                        ? new Set(staffWithAdminConnection.map((staff) => staff.id))
                                                                        : new Set(),
                                                                );
                                                            }}
                                                            // Ubah pengecekan untuk memastikan hanya staff dengan AdminStaff yang dipertimbangkan
                                                            checked={
                                                                selectedUsers.size > 0 &&
                                                                users.filter((staff) => staff.admin_staff).length === selectedUsers.size
                                                            }
                                                            className="h-4 w-4 cursor-pointer"
                                                        />
                                                    </motion.div>
                                                </th>
                                                <th scope="col" className="p-2 text-left text-xs font-medium sm:p-3 sm:text-sm">
                                                    Nama
                                                </th>
                                                <th scope="col" className="p-2 text-left text-xs font-medium sm:p-3 sm:text-sm">
                                                    Email
                                                </th>
                                                <th scope="col" className="p-2 text-center text-xs font-medium sm:p-3 sm:text-sm">
                                                    NIP
                                                </th>
                                                <th scope="col" className="p-2 text-center text-xs font-medium sm:p-3 sm:text-sm">
                                                    NUPTK
                                                </th>
                                                <th scope="col" className="p-2 text-left text-xs font-medium sm:p-3 sm:text-sm lg:table-cell">
                                                    Bagian
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                            <AnimatePresence initial={false}>
                                                {sortedUsers.filter((staff) => staff.admin_staff).length > 0 ? (
                                                    sortedUsers
                                                        .filter((staff) => staff.admin_staff) // Filter hanya staff yang memiliki admin_staff
                                                        .map((staff, index) => (
                                                            <motion.tr
                                                                key={staff.id}
                                                                custom={index}
                                                            ...
                                                            >
                                                                ...
                                                                <td className="p-2 text-xs text-gray-900 sm:p-3 sm:text-sm dark:text-gray-200">
                                                                    {staff.user.name}
                                                                </td>
                                                                <td className="p-2 text-xs text-gray-900 sm:p-3 sm:text-sm dark:text-gray-200">
                                                                    {staff.user.email}
                                                                </td>
                                                                <td className="p-2 text-center text-xs text-gray-900 sm:p-3 sm:text-sm dark:text-gray-200">
                                                                    {staff.nip || '-'}
                                                                </td>
                                                                <td className="p-2 text-center text-xs text-gray-900 sm:p-3 sm:text-sm dark:text-gray-200">
                                                                    {staff.nuptk || '-'}
                                                                ...
                                                                <td className="p-2 text-xs text-gray-900 sm:p-3 sm:text-sm dark:text-gray-200">
                                                                    {staff.admin_staff?.name || 'Tidak ada'}
                                                                </td>
                                                            </motion.tr>
                                                        ))
                                                ) : (
                    ...
                    <div className="mt-4 md:hidden">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            <motion.div whileTap={{ scale: 0.95 }}>
                                <div className="flex items-center">
                                    <input
                                        type="checkbox"
                                        onChange={(e) => setSelectedUsers(e.target.checked ? new Set(users.map((staff) => staff.id)) : new Set())}
                        ...
                        <AnimatePresence initial={false}>
                            {sortedUsers.filter((staff) => staff.admin_staff).length > 0 ? (
                                sortedUsers
                                    .filter((staff) => staff.admin_staff) // Filter staff yang punya admin_staff
                                    .map((staff, index) => (
                                        <motion.div
                                            key={staff.id || index}
                                            custom={index}
                                            ...
                                            className="mb-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900"
                                        >
                                            <div className="mb-2 flex items-center justify-between">
                                                <span className="text-sm font-medium text-gray-900 dark:text-white">{staff.user.name}</span>
                                                <motion.div
                                                    animate={selectedUsers.has(staff.id) ? 'checked' : 'unchecked'}
                                                    variants={checkboxVariants}
                                                    whileTap={{ scale: 0.9 }}
                                            ...
                                            <div className="mb-2 text-xs text-gray-600 dark:text-gray-400">{staff.user.email}</div>
                                ...
                                <motion.div
                                    ...
                                    className="rounded-lg border border-gray-200 p-4 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400"
                                >
                                    Tidak ada data.
                                </motion.div>
                            )}
            ...
            <AnimatePresence>
                {showConfirmDialog && (
                    ...
                    >
                        <motion.div
                            className="w-full max-w-sm rounded-lg bg-white p-4 shadow-xl sm:max-w-md sm:p-6 dark:bg-gray-800"
                            variants={modalContentVariants}
                        >
                            <h3 className="text-base font-semibold text-gray-900 sm:text-lg dark:text-white">Konfirmasi Penghapusan</h3>
                            <div className="mt-3 sm:mt-4">
                                <p className="text-sm text-gray-600 sm:text-base dark:text-gray-300">
                                    Apakah Anda yakin ingin menghapus {selectedUsers.size} data pengguna yang dipilih?
                                </p>
                                <p className="mt-2 text-xs text-red-500 sm:text-sm">Tindakan ini tidak dapat dibatalkan!</p>
                            </div>
                            <div className="mt-4 flex flex-col-reverse gap-2 sm:mt-6 sm:flex-row sm:justify-end sm:space-x-2">
                                <motion.button
                                    onClick={cancelDelete}
                                    className="mt-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 sm:mt-0 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
